...
def create_figure(s):
    ...
    p.circle(x=x.value, y=y.value, color='c', size='sz', source=s, line_color="white", alpha=0.6, hover_color='white', hover_alpha=0.5)
...
def create_source():
    ...
    if size.value != 'None':
        ...
        df['sz'] = [SIZES[xx] for xx in groups.codes]

    df['c'] = "#31AADE"
    if color.value != 'None':
        groups = pd.qcut(df[color.value].values, len(COLORS))
        df['c'] = [COLORS[xx] for xx in groups.codes]
...
def update_plot(attr, old, new):
    layout.children[1] = create_figure(source)
...
layout = row(controls, create_figure(source))
